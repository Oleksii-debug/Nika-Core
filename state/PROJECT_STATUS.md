...
- PF2 PR #98 — durable ProductProject-to-coordinator checkpoint binding, exact head `bba309b4713d385e6b7a653fffbdec313866d499`, integrated after Core #686 + M12 #454 as merge `84998ea814fc5f99489ad583ab052b06cab2f12b`.
...
### PF2 — orchestration
PRs #92/#93/#94/#97/#98 are **INTEGRATED**.

The integrated PF2 surface now includes durable ProductProject identity/spec/row-version binding for coordinator checkpoints and fail-closed stale resume. PF5 may consume that public binding in later journey work; PR #100 does not duplicate or modify it.
...
Open PF3 follow-up #99, `auto-pf3/credential-broker-foundation`, current-main compatibility head `50a7083f6931e240b1b529073aba47138999188a`, remains **NOT INTEGRATED** while fresh Core #695/M12 #463 run. PF5 does not import credential-broker internals, raw credential material or unmerged sibling contracts.
...
Current real PF5 code/evidence PR is #100, `auto-pf5/project-scoped-command-center`, rebuilt linearly from exact current main `84998ea814fc5f99489ad583ab052b06cab2f12b`. Rollback history of its prior current-main candidate is preserved at `backup/auto-pf5-100-118e41d9`.
...
The superseded #100 head `118e41d9866e6e872ee3a91b5f827c0eb4fe4013` passed Core Ubuntu/Windows and M12 Ubuntu/Windows integrated verification while M12 packaged proof was still running, but main then advanced through PF2 #98. It receives no merge credit after the current-main rebuild. Fresh exact-head Core + M12 are required on the final rebuilt #100 candidate.
...
No Product Factory Windows candidate is promoted from PF5 #100. Package/release work starts only at a meaningful integrated exact-SHA milestone. Any M12 package produced while testing an isolated PF5 backend candidate is CI evidence only and is not a human-bound Product Factory release. The shared packaged WebView2/UIA gate remains mandatory and is not permission for PF5 to weaken accessibility checks.

## Next dependency-ordered wave

1. PF5 proves rebuilt #100 on its final exact head with fresh Core + M12, then rechecks live-main compatibility before merge.
...
4. PF5 may then connect integrated PF2 checkpoint identity and later PF3 credential-reference presentation without duplicating either owner.
