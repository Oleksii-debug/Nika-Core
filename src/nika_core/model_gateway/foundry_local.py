...
class FoundryLocalProvider:
    ...
    native work across every provider instance in the process. The upstream
    non-streaming Python API does not expose a proven hard-cancel primitive, so
    a timed-out native inference keeps process-wide ownership until its worker
    actually exits.
    ...
    async def complete(self, request: ModelRequest) -> ModelResponse:
        ...
        acquired = False
        worker: asyncio.Future[tuple[str, str, ModelUsage]] | None = None
        abandon_before_native_start = Event()
        try:
            ...
            worker = loop.run_in_executor(
                _FOUNDRY_NATIVE_EXECUTOR,
                self._complete_sync,
                request,
                abandon_before_native_start,
            )
            try:
                ...
                abandon_before_native_start.set()
                self._release_slot_when_worker_finishes(worker)
                acquired = False
                raise ModelGatewayError(
                    ModelErrorCode.TIMEOUT,
                    (
                        "Foundry Local inference timed out; any already-started native "
                        "inference keeps the process-wide provider slot until it exits"
                    ),
    ...
    async def download_model(
        ...
        *,
        ...
        and retains shared provider/model-management ownership until the queued
        or running native operation has actually exited.
        """
        ...
        worker: asyncio.Future[FoundryModelEvidence] | None = None
            ...
            try:
                ...
                raise ModelGatewayError(
                    ...
                    (
                        ...
                        "cancellation was signalled and ownership is retained until native exit"
                    ),
                    provider_id=self.capabilities.provider_id,
                    retryable=False,
                ) from exc
    ...
    def inspect_model(self, model_alias: str | None = None) -> FoundryModelEvidence:
        """Return stable read-only public-SDK metadata for release/hardware evidence."""
    ...
    def close(self) -> None:
        ...
        FoundryLocalManager is a process-wide singleton. Closing while any native
        Foundry operation is active fails closed, including work started by a
        different provider instance. That prevents an owner from unloading a
        model underneath another in-process consumer.
        """
        ...
        if not _FOUNDRY_NATIVE_LOCK.acquire(blocking=False):
            raise RuntimeError(
                "cannot close Foundry Local provider while process-wide native work is active"
            )
    ...
    def _complete_sync(
        ...
        with _FOUNDRY_NATIVE_LOCK:
            if abandon_before_native_start.is_set():
                raise ModelGatewayError(
                    ModelErrorCode.CANCELLED,
                    "Foundry Local inference was abandoned before native execution started",
                    provider_id=self.capabilities.provider_id,
                    retryable=False,
                )
            ...
            if abandon_before_native_start.is_set():
                raise ModelGatewayError(
                    ModelErrorCode.CANCELLED,
                    "Foundry Local inference was abandoned before model execution started",
                    provider_id=self.capabilities.provider_id,
                    retryable=False,
                )
            ...
            if abandon_before_native_start.is_set():
                raise ModelGatewayError(
                    ModelErrorCode.CANCELLED,
                    "Foundry Local inference was abandoned before chat execution started",
                    provider_id=self.capabilities.provider_id,
                    retryable=False,
                )
    ...
    def _usage(response: Any) -> ModelUsage:
        ...
        def read(*names: str) -> int | None:
            for name in names:
                ...
                if isinstance(value, bool) or not isinstance(value, int) or value < 0:
                    raise TypeError(
                        f"Foundry Local usage field '{name}' must be a non-negative int"
                    )
                return value
            return None
        ...
        if (
            ...
            raise ValueError("Foundry Local usage total_tokens is inconsistent")
