...
class ModelGatewayAgentRuntime:
    """Thin AgentRuntimePort adapter for one exact local ModelGateway route.
    ...
    multi-agent supervisor use the already-integrated ModelGateway contract.
    """
        ...
        return frozenset(
            {
                ...
                RuntimeCapability.LOCAL_MODELS,
            }
        )
    ...
    async def run(self, request: RuntimeRequest) -> RuntimeResult:
        try:
            ...
            return RuntimeResult(
                ...
                error="The agent request is invalid and cannot be sent to the local model.",
        ...
        async with self._active_lock:
            ...
            if existing is not None and not existing.done():
                return RuntimeResult(
                    ...
                    error="This agent already has an active local model request.",
        ...
        try:
            ...
            return RuntimeResult(
                ...
                output={
                    "message": "Local model request cancelled.",
        ...
        if response.request_id != model_request.request_id:
            return RuntimeResult(
                ...
                error="The local model returned a response for a different request.",
                error_code=RuntimeErrorCode.INTERNAL,
            )
        if response.provider_id != self._provider_id or response.provider_kind is not ProviderKind.LOCAL:
            return RuntimeResult(
                outcome=RuntimeOutcome.FAILED,
                output={
                    "recoverable": False,
                ...
                error="The local model response did not match the configured provider route.",
                error_code=RuntimeErrorCode.INTERNAL,
            )
    ...
    async def resume(self, request: RuntimeResumeRequest) -> RuntimeResult:
        ...
        return RuntimeResult(
            ...
            output={"recoverable": True, "provider_id": self._provider_id},
            error=(
                "This local model request cannot resume from an opaque inference checkpoint. "
    ...
    def _failure_result(self, error: ModelGatewayError) -> RuntimeResult:
        message = {
            ...
            ModelErrorCode.TIMEOUT: "The local model request timed out. Retry when the provider is ready.",
            ModelErrorCode.CANCELLED: "The local model request was cancelled.",
            ModelErrorCode.RESOURCE_LIMIT: (
                "The local model cannot run within the current resource limits. "
            ...
            ModelErrorCode.INVALID_REQUEST: (
                "The configured local model route rejected this request. Check the model configuration."
            ),
            ModelErrorCode.PROVIDER_ERROR: (
                "The local model provider could not complete the request. "
