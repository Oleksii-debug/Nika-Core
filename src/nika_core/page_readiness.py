...
async def observe_page_readiness(
    ...
    *,
    ...
    if not callable(sample) or not callable(clock) or not callable(sleeper):
        raise TypeError("sample, clock, and sleeper must be callable")
    ...
    for observation_number in range(1, max_observations + 1):
        ...
        if observation_number > 1:
            ...
            if (
                ...
                raise ValueError("clock must return a finite monotonic value")
            # Preserve the incumbent exact-deadline contract: an observation at t == deadline
            # is permitted, but scheduler/sleeper overshoot must never authorize post-deadline work.
