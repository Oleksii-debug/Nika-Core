(() => {
  ...
  function renderApprovals(items) {
    ...
    for (const item of items) {
      ...
      heading.textContent = `${item.action_id} — ризик ${item.risk}`;
      ...
      approve.type = "button";
      approve.textContent = "Підтвердити цю точну дію";
      approve.dataset.actionId = "approval.approve";
      approve.dataset.approvalRequestId = item.request_id;
      approve.setAttribute("aria-label", `Підтвердити точну дію ${item.action_id}: ${item.target}`);
      const deny = document.createElement("button");
      deny.type = "button";
      deny.textContent = "Відхилити";
      deny.dataset.actionId = "approval.deny";
      deny.dataset.approvalRequestId = item.request_id;
      deny.setAttribute("aria-label", `Відхилити дію ${item.action_id}: ${item.target}`);
  ...
  async function dispatch(actionId, trigger = null) {
    ...
    if (actionId === "approval.approve" || actionId === "approval.deny") {
      const approvalRequestId = trigger?.dataset?.approvalRequestId;
      if (!approvalRequestId) {
        announce("Запит на людське підтвердження не визначено; дію відхилено.", true);
        return;
      }
