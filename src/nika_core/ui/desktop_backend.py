...
class DesktopBackend:
    ...
    R4 approval state is owned by the trusted Python process. The web UI receives only safe
    request views and submits request IDs back to approve/deny handlers; signatures and evidence
    never cross the pywebview bridge.
    """
    ...
    def stop_agent(self, _payload: Mapping[str, Any]) -> UIResult:
        ...
        elif record.state in _LOCAL_CANCEL_STATES:
            self._queue.transition(record.task_id, TaskState.CANCELLED)
        else:
            raise ValueError(
                "Неможливо безпечно скасувати активне runtime-завдання без збереженої сесії."
            )
    ...
    def approve_action(self, payload: Mapping[str, Any]) -> UIResult:
        ...
        return UIResult(
            request_id="desktop-handler",
            status="completed",
            message="Небезпечну дію явно підтверджено. Підтвердження діє лише для цієї точної дії.",
    ...
    def snapshot(self) -> dict[str, Any]:
        approvals = (
            ...
            else [item.as_dict() for item in self._approval_authority.pending_views()]
        )
        return {
