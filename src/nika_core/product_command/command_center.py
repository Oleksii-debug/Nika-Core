    ...
    def inspect_project(
        ...
        *,
        ...
        if execution is not None:
            ...
            statuses.extend(
                execution_status_entries(_scope_execution(project_id, execution))
            )
...
def _validate_deployment_snapshot(snapshot: DeploymentFabricSnapshot) -> None:
    ...
    for item in snapshot.healthy_staging:
        if len(item) != 2:
            raise ProductCommandCenterScopeError(
                "deployment snapshot contains malformed healthy-staging identity"
            )
        project_id, _sha = item
        if not project_id.strip():
            raise ProductCommandCenterScopeError(
                "deployment snapshot contains empty healthy-staging project identity"
            )
        staging_keys.append(project_id)
    if len(staging_keys) != len(set(staging_keys)):
        raise ProductCommandCenterScopeError(
            "deployment snapshot contains duplicate healthy-staging project identities"
        )

    current_release_keys: list[tuple[str, str]] = []
    for item in snapshot.current_releases:
        if len(item) != 3:
            raise ProductCommandCenterScopeError(
                "deployment snapshot contains malformed current-release identity"
            )
        project_id, environment_id, _sha = item
        if not project_id.strip() or not environment_id.strip():
            raise ProductCommandCenterScopeError(
                "deployment snapshot contains empty current-release identity"
            )
        current_release_keys.append((project_id, environment_id))
    if len(current_release_keys) != len(set(current_release_keys)):
        raise ProductCommandCenterScopeError(
            "deployment snapshot contains duplicate project/environment release identity"
        )
    ...
    for record in snapshot.records:
        ...
        if (
            record.rollback is not None
            and record.rollback.succeeded
            and record.rollback.restored_release_sha != record.previous_release_sha
        ):
            raise ProductCommandCenterScopeError(
                "successful rollback evidence does not restore the recorded previous release"
            )
...
def _scope_deployment(
    ...
    current_releases = tuple(
        item
        for item in snapshot.current_releases
        if item[0] == project_id and item[1] in environment_ids
    )
...
def _require_unique_status_identity(statuses) -> None:
    ...
    for item in statuses:
        ...
        if identity in seen:
            raise ProductCommandCenterScopeError(
                "duplicate ProductProject status identity: "
