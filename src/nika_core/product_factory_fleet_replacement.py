...
class FleetReplacementPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(value.strip() for value in identities):
            raise FleetReplacementError("replacement plan identity/approval must not be empty")
...
class FleetReplacementCoordinator:
    ...
    def submit(self, plan: FleetReplacementPlan) -> FleetReplacementRecord:
        ...
        *,
        ...
        for key in self._ordered_keys(plan):
            ...
            if record.state is ReplicaReplacementState.SUCCEEDED:
                continue
            if record.state is ReplicaReplacementState.FAILED:
                continue
        ...
        for key in self._ordered_keys(plan):
            record = records[key]
            if record.state in {ReplicaReplacementState.SUCCEEDED, ReplicaReplacementState.FAILED}:
                continue
            ...
            if orphan is not None and orphan.expires_at > instant:
                records[key] = replace(
                    record, state=ReplicaReplacementState.WAITING_FOR_ORPHAN_LEASE
                )
                continue
            ...
            if self._source_has_other_active_lease(spec.source_node_id, spec.request.work_id):
                records[key] = replace(
                    record, state=ReplicaReplacementState.WAITING_FOR_SOURCE_LEASE
                )
                continue
            ...
            request = self._request(plan, record, lease)
            checkpoint = replace(
                record,
            ...
            try:
                ...
                records[key] = replace(checkpoint, state=ReplicaReplacementState.RECONCILE_REQUIRED)
    ...
    def restore(self, snapshot: FleetReplacementSnapshot) -> None:
    ...
    def _bind(
        ...
        ops_replica = next(
            (item for item in operations.service.replicas if item.replica_id == spec.replica_id),
            None,
        )
    ...
    def _budget_allows(
        ...
        unavailable_nodes = set(self.operations.snapshot().unavailable_nodes)
        unavailable_environment = 0
        unavailable_service = 0
        service = self._service(record.binding.spec.service_id)
        for service_record in self.operations.snapshot().services:
    ...
    def _request(
        ...
        return ReplicaReplacementRequest(
            ...
            evidence_refs=plan.evidence_refs + (f"execution-node:{lease.node_id}",),
        )
    ...
    def _validate_pending_leases(
        ...
        for record in records.values():
            leases = self._matching_leases(
                plan.project_id, record.binding.spec.request.work_id
            )
            if len(leases) > 1:
    ...
    def _matching_leases(self, project_id: str, work_id: str) -> tuple[WorkLease, ...]:
        return tuple(
            lease
            for lease in self.nodes.snapshot().leases
    ...
    def _node(self, node_id: str) -> ExecutionNode:
        matches = [
            node for node in self.nodes.snapshot().nodes if node.identity.node_id == node_id
        ]
        if len(matches) != 1:
...
def _validate_digest(value: str) -> None:
    if len(value) != 64 or any(character not in "0123456789abcdef" for character in value):
        raise FleetReplacementError("replacement artifact digest is invalid")
