...
class ResearchReportExporter:
        ...
        *,
        ...
        if report_format is ResearchReportFormat.TXT:
            return RenderedResearchReport(
                filename, "text/plain; charset=utf-8", _privacy_safe_report_text(report).encode()
            )
    ...
    def _html(self, report: AccessibleResearchReport, *, language_tag: str) -> str:
        english_attr = _english_label_attr(language_tag)
        parts = [
            "<!doctype html>",
            ...
            for evidence in card.evidence:
                ...
                parts.extend(
                    [
                        ...
                        (
                            f"<dt{english_attr}>Location</dt><dd>"
                            f"{_escape(safe_evidence_location(evidence))}</dd>"
                        ),
        ...
        for position, card in enumerate(report.cards, start=1):
            ...
            if card.evidence:
                ...
                for evidence_index, evidence in enumerate(card.evidence, start=1):
                    ...
                    _add_labeled_paragraph(document, "Location", safe_evidence_location(evidence))
...
def _privacy_safe_report_text(report: AccessibleResearchReport) -> str:
    text = report.text
    for card in report.cards:
        for evidence in card.evidence:
            if evidence.locator:
                text = text.replace(
                    evidence.locator,
                    safe_evidence_location(evidence),
                )
    return text


def _english_label_attr(language_tag: str) -> str:
    primary_language = language_tag.split("-", maxsplit=1)[0].casefold()
    return "" if primary_language == "en" else ' lang="en"'
                ...
                row.update(
                    {
                        ...
                        "locator": safe_evidence_location(evidence),
