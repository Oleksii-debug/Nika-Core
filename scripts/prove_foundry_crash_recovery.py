...
from nika_core.model_gateway.foundry_local import (
...
class _CrashWindowProvider:
    """Publish a marker only while a real Foundry request remains unresolved."""
...
def _wait_for_crash_window(
    *,
    ...
    proof_dir: Path,
    timeout_seconds: float,
) -> tuple[dict[str, object], RuntimeSessionRecord]:
    paths = _proof_paths(proof_dir)
        ...
        if paths["ready"].exists() and paths["db"].exists():
            try:
                ...
                if paths["call_complete"].exists():
                    raise RuntimeError(
                        "native inference completed before the crash boundary was captured"
                    )
                ...
                if paths["call_complete"].exists():
                    raise RuntimeError(
                        "native inference completed before durable session validation"
                    )
...
def _require_same_active_session(
    before: RuntimeSessionRecord,
    after: RuntimeSessionRecord | None,
) -> RuntimeSessionRecord:
    if after is None or after.is_active is not True:
...
def _suspend_and_kill(
    ...
    *,
    proof_dir: Path,
    session_before_suspend: RuntimeSessionRecord,
) -> RuntimeSessionRecord:
    try:
        import psutil
    except ImportError as exc:
    ...
    try:
        ...
        if paths["call_complete"].exists():
            raise RuntimeError(
                "native inference completed before the suspended hard-kill boundary"
            )
        store = SQLiteStore(paths["db"])
        store.initialize()
        frozen = RuntimeSessionStore(store).get(session_before_suspend.task_id)
        frozen = _require_same_active_session(session_before_suspend, frozen)
        if paths["call_complete"].exists():
            raise RuntimeError(
                "native inference completed before frozen session validation"
            )
    except Exception:
        try:
            target.resume()
            ...
            raise RuntimeError(
                "failed to resume crash child after suspended-boundary validation failed"
            ) from exc
        raise
...
def _arm(args: argparse.Namespace) -> dict[str, object]:
    ...
    evidence: dict[str, object] = {
        ...
        "resume_token_sha256": _resume_token_sha256(session.resume_token),
