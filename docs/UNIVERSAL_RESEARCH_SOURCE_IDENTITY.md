...
`HttpFetchResult.failure_class` is a stable provider-neutral classification layered on top of the existing `disposition` and `error_code` fields:
...
- `policy` — malformed/disallowed URL, host, port, userinfo, or insecure-HTTP policy failure;
...
- **ADAPT** — standard-library URL parsing/IDNA normalization is adapted only for source identity.
- **CUSTOM (thin)** — Nika-specific immutable source identity errors and the small stable fetch-failure enum.
