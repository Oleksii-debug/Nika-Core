...
`ProductSearchService.render_text()` emits linear text with a stable field order: title, seller, price, availability, match reason, source, observation time, and snippet. It does not require color, cards, pointer interaction, spatial interpretation, or a visual table.
...
3. Focused Product Search tests pass.
