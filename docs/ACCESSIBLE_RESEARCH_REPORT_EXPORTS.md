...
## Current-main privacy repair

User-facing report artifacts deliberately do not disclose `ResearchEvidence.locator`.
The locator remains internal canonical provenance because it can contain HTTP userinfo,
signed or secret query/fragment material, or a private local filesystem path. The
accessible text report and TXT/CSV/HTML/DOCX/XLSX exporters instead render the
deterministic marker `[location omitted for privacy]` while preserving source ID,
source kind, freshness and observed time.

This is one shared presentation-boundary policy, not a second provenance store or
network sanitizer. Callers that need the canonical raw locator must use the internal
typed evidence object under the ordinary permission/privacy boundary rather than a
shareable report artifact.

HTML exporter-owned English labels treat only the exact primary language subtag
`en` as inheriting English from the document. Other syntactically accepted primary
subtags receive an explicit `lang="en"` annotation on English labels.
