...
No schema migration, scheduler-contract edit, ML dependency, or new orchestration framework is added.
...
4. The deadline guard uses unlimited misfire grace so restart after downtime can still reconcile the
...
5. `register()` and `resume()` never reactivate a terminal monitor. Reuse the existing schedule ID
   only for the same canonical monitor; use a new ID for a new monitoring lifecycle.
6. Startup/recovery should call `reconcile()` for an active monitor. It repairs the derived deadline
