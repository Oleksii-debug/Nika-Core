...
## Current-main conformance repairs

The current-main successor closes the independent Worker54 findings without changing monitor
authority. Each `MonitoringCheck` now carries immutable controller-supplied
`next_scheduled_check` and `terminal_reason` snapshots. The renderer uses one deterministic
per-check field order and emits a machine-generated, body-free `Evidence summary` derived only
from counts of normalized change/provenance/source references.

Source outcomes are canonicalized by stable `(source_kind, source_id)` identity and duplicate
identities fail closed. Error/terminal codes are restricted to bounded machine-code syntax rather
than arbitrary free-form text. Authorization/Cookie header material is redacted as a whole, and
token/password assignment forms remain redacted. The projection still omits raw URLs, locators,
headers, cookies, snippets, bodies, and free-form network errors.
