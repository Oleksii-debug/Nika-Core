...
Every query has an explicit integer `limit` in the closed range 1..100. The SQL projection preserves deterministic `ORDER BY rank, document_id` ordering and reads at most `limit + 1` rows. The one bounded lookahead row is used only to determine whether more matching records existed; at most `limit` hits are passed to result persistence or downstream context.

`ResearchQueryExecution.bounds` exposes `requested_limit`, `returned_count`, and `truncated`. Therefore a caller can distinguish an exact result set from one intentionally bounded by policy without materializing or counting an unbounded match set. The text renderer exposes the same limit/count/truncation evidence.
...
The query execution wrapper retains the requested mode/filter specification and result-bound evidence for immediate deterministic rendering. Versioned reusable profile/source-set definitions are persisted separately through canonical migration 12 and reuse this same query contract.

The text renderer exposes query mode, active filters, requested limit, returned result count, truncation state, title/snippet and source provenance as ordinary text suitable for screen-reader consumption. It does not award `HUMAN_TESTED` or `NVDA_VERIFIED`.
...
- CUSTOM (thin): Nika-owned query specification, safe FTS construction, workspace/source policy filters, bounded lookahead evidence and accessible text projection.

No new dependency is introduced by this batch.

## Failure policy

The API fails closed for an empty workspace/query, non-integer limits, limits outside 1..100, empty source/media filter values, unknown source IDs, cross-workspace source IDs, and freshness filters that explicitly exclude HTTP sources. SQL values are parameterized; only placeholder counts and fixed query fragments are constructed by code.
