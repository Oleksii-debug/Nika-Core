...
def test_private_route_fails_before_credential_resolution_or_network() -> None:
    ...
    provider = CredentialRefOpenAICompatibleProvider(
        config=_config(supports_private_data=False),
    ...
    with pytest.raises(ModelGatewayError) as caught:
        asyncio.run(gateway.complete(_request(privacy=PrivacyClass.PRIVATE)))
