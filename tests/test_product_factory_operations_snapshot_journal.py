...
def _snapshot(result: MaintenanceResult) -> ProductOperationsSnapshot:
    return ProductOperationsSnapshot(
        ...
        maintenance_records=(MaintenanceRecord(_request(), result),),
        revoked_credentials=(),
        unavailable_nodes=(),
    )
...
def _coordinator(journal: RuntimeIdempotencyMaintenanceJournal) -> ProductOperationsCoordinator:
    return ProductOperationsCoordinator(
        "project-a",
...
def test_restore_rejects_applied_snapshot_when_canonical_journal_has_no_effect(tmp_path) -> None:
    ...
    with pytest.raises(ProductOperationsError, match="maintenance effect"):
        _coordinator(journal).restore(_snapshot(forged))
...
def test_restore_accepts_exact_completed_journal_result_without_mutation(tmp_path) -> None:
    _, ledger, journal = _journal(tmp_path)
    reservation = journal.reserve(
        project_id="project-a",
        service=_service(),
    ...
    durable = MaintenanceResult(True, False, ("provider:durable",))
    journal.complete(reservation.operation_key, durable)
    before = ledger.require(reservation.operation_key)

    coordinator = _coordinator(journal)
    coordinator.restore(_snapshot(durable))

    assert coordinator.snapshot() == _snapshot(durable)
    assert ledger.require(reservation.operation_key) == before
