...
def test_csv_preserves_review_provenance_and_blocks_formula_injection() -> None:
    ...
    assert row["locator"] == "[location omitted for privacy]"
...
def test_html_is_semantic_and_escapes_untrusted_text() -> None:
    ...
    assert "[location omitted for privacy]" in text
...
def test_docx_has_heading_hierarchy_labels_review_and_provenance() -> None:
    ...
    assert ("Normal", "Location: [location omitted for privacy]") in paragraphs
...
def test_xlsx_is_flat_accessible_and_blocks_formula_injection() -> None:
    ...
    assert values["locator"] == "[location omitted for privacy]"
...
def _private_locator_report() -> AccessibleResearchReport:
    http_locator = (
        "https://resolver-user:resolver-secret@example.org/private/path"
        "?access_token=HTTP-CANARY#fragment"
    )
    local_locator = r"C:\Users\Alice\Private\api_key=LOCAL-CANARY.txt"
    base = _report()
    card = base.cards[0]
    private_card = ResearchCard(
        ...
        evidence=(
            ResearchEvidence(
                source_id="source-http-private",
                source_kind=SourceKind.HTTP,
                locator=http_locator,
                observed_at="2026-08-20T07:00:00+00:00",
                freshness=FreshnessState.CURRENT,
            ),
            ResearchEvidence(
                source_id="source-local-private",
                source_kind=SourceKind.LOCAL_FILE,
                locator=local_locator,
                observed_at="2026-08-20T07:00:01+00:00",
                freshness=FreshnessState.CURRENT,
            ),
        ),
        review=card.review,
    ...
    return AccessibleResearchReport(
        ...
        text=(
            base.text
            + f"Location: {http_locator}\n"
            + f"Location: {local_locator}\n"
        ),
    )


def _rendered_text_for_privacy_assertion(
    report_format: ResearchReportFormat,
    content: bytes,
) -> str:
    if report_format in {
        ResearchReportFormat.TXT,
        ResearchReportFormat.CSV,
        ResearchReportFormat.HTML,
    }:
...
@pytest.mark.parametrize("report_format", tuple(ResearchReportFormat))
def test_all_formats_omit_raw_evidence_locator_secrets(
    report_format: ResearchReportFormat,
) -> None:
    kwargs = {"language_tag": "uk"} if report_format is ResearchReportFormat.HTML else {}
    rendered = ResearchReportExporter().render(
        _private_locator_report(),
        report_format,
        **kwargs,
    )
    text = _rendered_text_for_privacy_assertion(report_format, rendered.content)

    assert "resolver-user" not in text
    assert "resolver-secret" not in text
    assert "HTTP-CANARY" not in text
    assert "C:\\Users\\Alice\\Private" not in text
    assert "LOCAL-CANARY" not in text
    assert "[location omitted for privacy]" in text
    assert "source-http-private" in text
    assert "source-local-private" in text


def test_non_en_primary_language_prefix_does_not_hide_english_label_language() -> None:
    rendered = ResearchReportExporter().render(
        _report(),
        ResearchReportFormat.HTML,
        language_tag="ena",
    ...
    assert '<html lang="ena">' in text
    assert '<h1 lang="en">Research results</h1>' in text
    assert '<dt lang="en">Source ID</dt>' in text
