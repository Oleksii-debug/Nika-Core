...
def test_qa53_oracle_secret_content_in_benign_audit_fields_never_reaches_sqlite(
    ...
    """QA_ONLY: AuditLog must not persist embedded credential material by value.

    These are deliberately synthetic canaries, not real credentials.  The attack models
    exception/HTTP/provider/Git/media diagnostics entering a generic audit field whose
    key is not itself secret-bearing.
    """
...
def test_qa53_oracle_query_token_in_url_value_never_reaches_sqlite(tmp_path: Path) -> None:
    """QA_ONLY: cover the query component separately from URL userinfo."""
    ...
    audit.append(
        ...
        entity_id="qa53",
        payload={
            "url": "https://audit.invalid/path?access_token=" + canary + "&page=1"
        },
    )
