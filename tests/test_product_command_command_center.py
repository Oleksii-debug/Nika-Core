...
def _deployment_record(
    *,
    ...
    source_sha: str,
    digest: str,
    state: DeploymentState = DeploymentState.UNCERTAIN,
) -> DeploymentRecord:
    intent = DeploymentIntent(
        ...
        EnvironmentIdentity(
            ...
            f"provider://{project_id}/staging",
        ),
        ReleaseRef(project_id, "1.0.0", source_sha, digest),
    )
    return DeploymentRecord(intent, state, (f"deploy://{project_id}/evidence",))
...
def test_deployment_scoping_uses_project_plus_environment_identity(tmp_path) -> None:
    center = _center(tmp_path)
    ...
    snapshot = DeploymentFabricSnapshot(
        (target, foreign),
        (("p1", SHA_A), ("p2", SHA_B)),
        (("p1", "staging", SHA_A), ("p2", "staging", SHA_B)),
    )

    serialized = center.inspect_project("p1", deployment=snapshot).model_dump_json()
...
def test_duplicate_project_environment_current_release_fails_closed(tmp_path) -> None:
    ...
    snapshot = DeploymentFabricSnapshot(
        (),
        (),
        (("p1", "staging", SHA_A), ("p1", "staging", SHA_B)),
    )

    with pytest.raises(ProductCommandCenterScopeError, match="project/environment"):
        center.inspect_project("p1", deployment=snapshot)


def test_same_environment_text_for_different_projects_is_not_a_collision(tmp_path) -> None:
    center = _center(tmp_path)
    target = _deployment_record(
        project_id="p1",
        intent_id="target",
...
def test_malformed_current_release_shape_fails_closed(tmp_path) -> None:
    center = _center(tmp_path)
    snapshot = DeploymentFabricSnapshot((), (), (("staging", SHA_A),))

    with pytest.raises(ProductCommandCenterScopeError, match="malformed current-release"):
        center.inspect_project("p1", deployment=snapshot)
...
def test_health_evidence_must_match_environment_and_exact_release_sha(tmp_path) -> None:
    ...
    record = DeploymentRecord(
        ...
        ("deploy://ok",),
        health=HealthEvidence("other", SHA_A, True, ("health://ok",), NOW),
    )
...
def test_successful_rollback_must_restore_recorded_previous_release(tmp_path) -> None:
    ...
    with pytest.raises(ProductCommandCenterScopeError, match="recorded previous release"):
        center.inspect_project(
            "p1",
            deployment=DeploymentFabricSnapshot((record,), (), ()),
        )
