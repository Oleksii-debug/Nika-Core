...
def test_aud05_irregular_grid_cannot_claim_annualized_sharpe_or_sortino() -> None:
    points = (
...
def test_aud05_regular_grid_rejects_missing_period_instead_of_reannualizing() -> None:
...
def test_aud05_same_metric_name_with_changed_definition_fails_closed() -> None:
    p = protocol()
...
def test_aud05_same_quality_counts_with_different_evidence_fails_closed() -> None:
    p = protocol()
    selection = selected(p)
    report = ValidationReport(
        duplicates=(ValidationIssue("duplicate", "aud05 adversarial issue", (1, 2)),),
    )
...
def test_aud05_same_strategy_id_cannot_swap_config_at_heldout_boundary() -> None:
    p = protocol()
    selection = selected(p)
...
def test_aud05_refit_cannot_include_any_test_period_information() -> None:
...
def test_aud05_original_evidence_mutation_cannot_rewrite_bound_chronology() -> None:
    p = protocol()
    selection = selected(p)
...
def test_aud05_bound_snapshot_mutation_invalidates_promotion_authority() -> None:
    p = protocol()
    selection = selected(p)
...
def test_aud05_malformed_and_nonfinite_values_fail_closed() -> None:
...
def test_aud05_dev26_has_no_network_broker_or_order_execution_import() -> None:
