...
@pytest.mark.parametrize("change", ["target_id", "payload", "order", "batch_size"])
def test_restore_binds_current_workflow_input_before_any_effect(
    tmp_path: Path,
    change: str,
) -> None:
    memory, ledger, _ = _services(tmp_path)
    declared = [
        BatchTargetSpec(target_id="alpha", payload={"value": 1}),
        BatchTargetSpec(target_id="beta", payload={"value": 2}),
        BatchTargetSpec(target_id="gamma", payload={"value": 3}),
    ]
    BatchCursor.create(
        memory,
        ledger,
    ...
    current = list(declared)
    batch_size = 2
    if change == "target_id":
        current[1] = BatchTargetSpec(target_id="beta-changed", payload={"value": 2})
    elif change == "payload":
        current[1] = BatchTargetSpec(target_id="beta", payload={"value": 999})
    elif change == "order":
        current = [current[1], current[0], current[2]]
    else:
        batch_size = 1

    with pytest.raises(BatchCursorStateError, match="current workflow input"):
        BatchCursor.restore(
            memory,
            ledger,
            task_id="task-bind",
...
def test_crash_after_effect_completion_preserves_exact_inter_batch_wake(
    ...
    due = datetime(2031, 2, 3, 4, 5, 6, tzinfo=UTC)
    grant = cursor.begin_effect("target-0")
    assert grant.execute is True

    def crash_before_cursor_persist() -> None:
        raise SystemExit("simulated hard crash after ledger completion")

    monkeypatch.setattr(cursor, "_persist", crash_before_cursor_persist)
    ...
    assert restarted.state.targets[0].confirmed_result == {"remote_id": "done-0"}
    assert intent is not None
    assert intent.kind is IntentKind.INTER_BATCH_WAIT
