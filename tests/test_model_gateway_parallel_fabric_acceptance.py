...
def test_same_model_replicas_overlap_and_preserve_distinct_route_identity() -> None:
    async def scenario() -> tuple[int, tuple[str, ...], tuple[str, ...]]:
        overlap = _OverlapState()
        reached = asyncio.Event()
        release = asyncio.Event()
        gateway = ModelGateway()
        for provider_id in ("same-model-replica-a", "same-model-replica-b"):
            gateway.register(
                _BarrierCloudRoute(
                    provider_id=provider_id,
                    overlap=overlap,
        ...
        task = asyncio.create_task(
            complete_parallel(
                ...
                (
                    _request(
                        "replica-request-a",
                        provider_id="same-model-replica-a",
                        model="identical-model-id",
                    ),
                    _request(
                        "replica-request-b",
                        provider_id="same-model-replica-b",
                        model="identical-model-id",
                    ),
                ),
                max_parallel=2,
                provider_limits={
                    "same-model-replica-a": 1,
                    "same-model-replica-b": 1,
                },
            )
        )
        ...
        provider_ids = tuple(
            item.response.provider_id for item in result.completed if item.response is not None
        )
        model_ids = tuple(
            item.response.model for item in result.completed if item.response is not None
        )
        return observed, provider_ids, model_ids

    observed, provider_ids, model_ids = asyncio.run(scenario())

    assert observed == 2
    assert provider_ids == ("same-model-replica-a", "same-model-replica-b")
    assert model_ids == ("identical-model-id", "identical-model-id")
