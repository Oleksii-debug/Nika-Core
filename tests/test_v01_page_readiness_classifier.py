...
def test_classifier_covers_all_required_states() -> None:
    ...
    cases = [
        (
            {
                "snapshot": None,
                "locator": None,
                "signal": PageObservationSignal.LOADING,
            },
        ...
        (
            {
                "snapshot": None,
                "locator": None,
                "signal": PageObservationSignal.TEMPORARILY_BUSY,
            },
        ...
        ({"snapshot": ready, "locator": locator}, PageReadinessState.READY),
        ({"snapshot": _snapshot(), "locator": locator}, PageReadinessState.MISSING),
        (
            {
                "snapshot": _snapshot(_button(node_id="a"), _button(node_id="b")),
                "locator": locator,
            },
        ...
        (
            {
                "snapshot": None,
                "locator": None,
                "signal": PageObservationSignal.PAGE_CLOSED,
            },
            PageReadinessState.PAGE_CLOSED,
        ),
        ({"snapshot": None, "locator": locator}, PageReadinessState.VALIDATION_ERROR),
    ]
...
def test_oversleep_never_samples_after_hard_deadline() -> None:
    clock = _FakeClock()
    calls = 0
...
def test_exact_deadline_observation_remains_permitted() -> None:
    clock = _FakeClock()
