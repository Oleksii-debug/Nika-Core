...
@pytest.mark.parametrize("relation", ("equal", "destination-inside-data", "destination-contains-data"))
def test_install_rejects_default_canonical_data_overlap_before_mutation(
    tmp_path: Path,
    ...
    local_app_data = tmp_path / f"Local AppData {relation}"
    ...
    config = AppConfig()
    data_root = config.database_path.parent
    assert data_root == local_app_data / "NikaCore"

    if relation == "equal":
        destination = data_root
    elif relation == "destination-inside-data":
        destination = data_root / "Programs" / "NikaCore"
    else:
        destination = local_app_data

    bundle = _bundle(tmp_path / f"release-{relation}", "v1")
    result = _run(
        shell,
        mode="Install",
...
def test_install_rejects_explicit_database_authority_inside_destination(
    tmp_path: Path,
    alias: str,
) -> None:
    shell = _powershell()
    ...
    bundle = _bundle(tmp_path / f"release-{alias}", "v1")
    destination = tmp_path / f"Custom Install {alias}" / "Nika Core"
    explicit_database = destination / "durable" / "nika_core.db"
    env = os.environ.copy()
    env.pop("NIKA_DB_PATH", None)
    env.pop("NIKA_DATABASE_PATH", None)
    env[alias] = str(explicit_database)
...
@pytest.mark.skipif(os.name != "nt", reason="real PowerShell filesystem proof is Windows-only")
def test_normal_localappdata_programs_destination_remains_allowed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    shell = _powershell()
    ...
    local_app_data = tmp_path / "Local AppData Normal"
    local_app_data.mkdir(parents=True)
    monkeypatch.setenv("LOCALAPPDATA", str(local_app_data))
    monkeypatch.delenv("NIKA_DB_PATH", raising=False)
    monkeypatch.delenv("NIKA_DATABASE_PATH", raising=False)
    ...
    config = AppConfig()
    assert config.database_path.parent == local_app_data / "NikaCore"

    bundle = _bundle(tmp_path / "release-normal", "v1")
...
def test_failed_install_post_activation_verification_leaves_no_active_destination(
    ...
    bundle = _bundle(tmp_path / "release-install-fault", "v1")
...
def test_failed_third_rollback_move_restores_verified_precommand_pair(
    ...
    destination = tmp_path / "Rollback Root" / "Nika Core"
