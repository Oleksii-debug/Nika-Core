...
class _AllowExactReviewAuthority:
    def verify(
        self,
        *,
        project_id: str,
        ...
        return (
            ...
            and evidence_ref.startswith("review:")
            and bool(purpose)
        )
...
def _base_pf10_decision():
    dependency = DependencyAdoption(
        project_id="product-release-boundary",
        component_id="component-release-boundary",
        ...
        review_ref="review:license:example-package:1.0.0",
    )
    decision = ProductComplianceGate(
        review_authority=_AllowExactReviewAuthority(),
    ).evaluate(
        project_id="product-release-boundary",
        dependencies=(dependency,),
        dependency_closure_ref="review:dependency-closure:product-release-boundary",
...
def test_base_pf10_decision_cannot_bypass_exact_release_grant_at_delivery(
    tmp_path,
    business_authority,
) -> None:
    ...
    # A base PF10 decision is not a release authorization. Delivery must require
    # the integrity-bound ReleaseComplianceGrant issued only after exact artifact,
    # dependency, notice-bundle and current-release revalidation.
    with pytest.raises((BusinessFactoryError, TypeError)):
