...
def _handoff(
    ...
    *,
    ...
    package = ResearchEvidencePackage(
        package_id,
        (EvidenceRef(f"ev:{package_id}", f"research://{package_id}/claim/1", "Gap"),),
    )
    projects.record_research_handoff(
        "p1",
        package,
        (ProductOption(option_id, option_id, "Summary", (package_id, *extra_package_ids)),),
    )
...
def test_decision_is_durable_idempotent_and_restart_safe(tmp_path) -> None:
    ...
    _handoff(projects)
    stored = decisions.record("p1", _decision(), expected_row_version=0, idempotency_key="decision:approve:1")
    replay = decisions.record("p1", _decision(), expected_row_version=0, idempotency_key="decision:approve:1")
...
def test_decision_requires_exact_unambiguous_recorded_option(tmp_path) -> None:
    ...
    with pytest.raises(ProductProjectError, match="unknown product option"):
        decisions.record("p1", _decision(), expected_row_version=0, idempotency_key="decision:unknown")
    ...
    with pytest.raises(ProductProjectError, match="ambiguous product option"):
        decisions.record("p1", _decision(option_id="shared"), expected_row_version=0, idempotency_key="decision:ambiguous")
    assert projects.get("p1").row_version == 0
...
def test_proposed_decision_can_finalize_once_and_history_survives(tmp_path) -> None:
    _, projects, decisions = _repos(tmp_path)
    _handoff(projects)
    proposed = decisions.record("p1", _decision(state=ProductDecisionState.PROPOSED, rationale="Needs owner confirmation"), expected_row_version=0, idempotency_key="decision:proposed")
    approved = decisions.record("p1", _decision(), expected_row_version=1, idempotency_key="decision:approved")
    assert proposed.decision_version == 1
    assert approved.decision_version == 2
    assert [item.decision.state for item in decisions.history("p1", "decision-1")] == [ProductDecisionState.PROPOSED, ProductDecisionState.APPROVED]
    with pytest.raises(ProductProjectError, match="immutable"):
        decisions.record("p1", _decision(state=ProductDecisionState.REJECTED, rationale="Changed mind"), expected_row_version=2, idempotency_key="decision:rewrite-final")
...
def test_decision_option_identity_cannot_change_during_finalization(tmp_path) -> None:
    ...
    decisions.record("p1", _decision(state=ProductDecisionState.PROPOSED, rationale="Review"), expected_row_version=0, idempotency_key="decision:proposed:fixed-option")
    with pytest.raises(ProductProjectError, match="option cannot change"):
        decisions.record("p1", _decision(option_id="option-2"), expected_row_version=1, idempotency_key="decision:changed-option")
    assert projects.get("p1").row_version == 1
...
def test_only_one_current_option_can_be_approved(tmp_path) -> None:
    ...
    _handoff(projects, package_id="research-1", option_id="option-1")
    _handoff(projects, package_id="research-2", option_id="option-2")
    decisions.record("p1", _decision(decision_id="decision-1", option_id="option-1"), expected_row_version=0, idempotency_key="decision:1")
    with pytest.raises(ProductProjectError, match="already has approved option"):
        decisions.record("p1", _decision(decision_id="decision-2", option_id="option-2"), expected_row_version=1, idempotency_key="decision:2")
    assert projects.get("p1").row_version == 1
...
def test_requirement_link_is_explicit_versioned_and_idempotent(tmp_path) -> None:
    store, projects, decisions = _repos(tmp_path)
    _handoff(projects)
    decisions.record("p1", _decision(), expected_row_version=0, idempotency_key="decision:approve")
    linked = decisions.link_requirement("p1", requirement_id="req-1", decision_id="decision-1", expected_row_version=1)
    replay = decisions.link_requirement("p1", requirement_id="req-1", decision_id="decision-1", expected_row_version=1)
...
def test_requirement_link_rejects_proposed_and_stale_decisions(tmp_path) -> None:
    _, projects, decisions = _repos(tmp_path)
    _handoff(projects)
    decisions.record("p1", _decision(state=ProductDecisionState.PROPOSED, rationale="Review"), expected_row_version=0, idempotency_key="decision:proposed:requirement")
    with pytest.raises(ProductProjectError, match="requires approved"):
        decisions.link_requirement("p1", requirement_id="req-1", decision_id="decision-1", expected_row_version=1)
    with pytest.raises(StaleProjectVersionError):
        decisions.link_requirement("p1", requirement_id="req-1", decision_id="decision-1", expected_row_version=0)
...
def test_decision_stale_write_and_idempotency_conflict_fail_closed(tmp_path) -> None:
    _, projects, decisions = _repos(tmp_path)
    _handoff(projects)
    stored = decisions.record("p1", _decision(state=ProductDecisionState.PROPOSED, rationale="Review"), expected_row_version=0, idempotency_key="decision:key")
    assert stored.decision_version == 1
    with pytest.raises(ProductProjectError, match="different mutation input"):
        decisions.record("p1", _decision(state=ProductDecisionState.APPROVED), expected_row_version=1, idempotency_key="decision:key")
    with pytest.raises(StaleProjectVersionError):
        decisions.record("p1", _decision(decision_id="decision-2", state=ProductDecisionState.REJECTED), expected_row_version=0, idempotency_key="decision:stale")
    assert projects.get("p1").row_version == 1
...
def test_product_project_schema_v1_upgrades_without_data_loss(tmp_path) -> None:
    ...
    with store.connection() as conn:
        ...
        conn.execute("INSERT INTO schema_migrations(version, applied_at) VALUES (1, 'existing')")
        ...
        conn.execute("CREATE TABLE product_project_schema_migrations(version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL)")
        for statement in PRODUCT_PROJECT_MIGRATIONS[1]:
            conn.execute(statement)
        conn.execute("INSERT INTO product_project_schema_migrations(version, applied_at) VALUES (1, 'existing')")
        conn.execute("INSERT INTO product_projects(project_id,name,current_spec_version,row_version,status,created_at,updated_at) VALUES ('p1','Existing',1,0,'active','x','x')")
        conn.execute(
            "INSERT INTO product_project_specs(project_id,spec_version,spec_json,created_at) VALUES ('p1',1,?, 'x')",
            ('{"goal":"g","desired_outcome":"o","hypothesis":"","requirements":[],"architecture_decision_refs":[],"repository_refs":[],"team_refs":[],"artifact_refs":[],"build_refs":[],"release_refs":[],"deployment_refs":[],"incident_refs":[],"credential_refs":[],"budget":{},"risk":{},"compliance":{}}',),
        )
    store.initialize()
